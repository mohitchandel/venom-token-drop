...
class ViewTracingTree {
    constructor(viewTraceTree, contractGetter, accounts) {
        ...
        this.getErrorsByContract = (contract) => {
            return this.msgErrorsStore[(0, utils_1.extractStringAddress)(contract)];
        };
        this.getAllErrors = () => Object.entries(this.msgErrorsStore).flatMap(([key, errors]) => errors.map(error => ({ contract: key, ...error })));
        this.getBalanceDiff = (contracts) => {
        ...
        this.findCallsForContract = ({ contract, name, }) => {
            return this.findForContract({ contract, name })
                .map(el => el?.params)
                .filter(utils_1.isT);
        };
        this.findEventsForContract = ({ contract, name, }) => this.findForContract({ name, contract })
            .map(el => el?.params)
            .filter(utils_1.isT);
        this.findForContract = ({ contract, name, }) => {
        ...
        this.findByType = (params) => this._findByType(params, this.viewTraceTree).map(el => el.decodedMsg);
        this.findByTypeWithFullData = (params) => {
            return this._findByType(params, this.viewTraceTree);
        };
        this._findByType = ({ type, name, contract }, tree, isRoot = true) => {
        ...
        this.totalGasUsed = () => (0, utils_2.calculateTotalFees)(this.viewTraceTree).toNumber();
        this.beautyPrint = async (printerConfig) => {
        ...
        this._beautyPrint = (viewTrace, offset, contracts, printerConfig) => {
            let traces = "";
